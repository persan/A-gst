...
Initial_Code_Generation:
	...
	cd /usr/include/gstreamer-1.0; find -name "*.h" | grep -v -e vdpau/ -e pygst | sed -e "s-^./-#include <-" -e "s-\.h-.h>-" >>${CURDIR}/.gen/gen.cc
	echo "" >>.gen/gen.cc
	echo "}" >>.gen/gen.cc
	cd .gen; gcc ${CFLAGS} -c gen.cc
	rm -rf src/gen
	mkdir -p src/gen
	cp  .gen/*gstreamer_1_0_gst_*.ads src/gen
	sed "s!/usr/include/gstreamer-1.0/!!g" src/gen/*.ads -i
